...
namespace KiwiBankomaten
{
    public class Customer : User
    {
        ...
        public void OpenAccountForTest(decimal interest, string accountname, string currency)
        {
            int index = BankAccounts.Keys.Max() + 1;
            BankAccounts.Add(index, new BankAccount(accountname, currency, interest));
            ...
            int fromAccountNum, decimal amountMoney)
        {
            ...
            if (amountMoney >= 0)
            {
                ...
                foreach (Customer c in DataBase.CustomerDict.Values)
                {
                    foreach (BankAccount b in c.BankAccounts.Values)
                    {
                        if (fromAccountNum == b.AccountNumber)
                        {
                        ...
                        else if (toAccountNum == b.AccountNumber)
                        {
                ...
                if (b1.Amount >= amountMoney)
                {
                    b1.Amount -= amountMoney;
                    b1.LogList.Add(new Log(amountMoney, fromAccountNum, toAccountNum));
                    b2.Amount += (amountMoney / toRate) * fromRate;
                    b2.LogList.Add(new Log(amountMoney, fromAccountNum));
                }
            }
